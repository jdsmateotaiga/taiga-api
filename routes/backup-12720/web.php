...
Route::get('getrelatedproducts',['middleware'=>'cors',function(){
  date_default_timezone_set("Asia/Singapore");
    ...
    $tag = \App\Tags::where('product_id',$product_id)->orderBy('tag_ranking','ASC')->get();
    ...
    if(!empty($tag)){
      foreach($tag as $t){
        if(!empty($t->tag)){
          ...
          $sqlrel = 'SELECT
                          t_tags.product_id,
                          CONCAT(t_brands.brand," ",t_products.name) AS name,
                          t_variations.sku,
                          ...
                          AND t_tags.product_id<>'.$product_id.'
                          GROUP BY t_tags.product_id
                          ORDER BY RAND ()
                          LIMIT 18';

            $products = DB::select($sqlrel);
            if(!empty($products)){

              $x=0;
              foreach($products as $p){

                $saleresponse = checkifsale($p->product_id, date('Y-m-d'), date('Y-m-d'), $i = 'product');
                if(!empty($saleresponse)){
                  //get the additional discount, this is to be added on product savings
                  foreach($saleresponse as $sr){
                    if(!empty($sr->percentage) and $sr->percentage!=0){
                ...
                //===============================
                $productprice = round(trim($p->price),0);
                $totalsavings = $savings;
                $srp = round(trim($p->retail_price),0);

                if($onsale==true){
                  $price = round(trim($p->price),0);
                  $salepercent = strlen($addsale)>=2 ? ".".$addsale : ".0".$addsale;
                  $additionaldisc = round($salepercent * $price,0);
                  $productprice = $price - $additionaldisc;
                  $totalsavings = round((($srp - $productprice) / $srp) * 100,0);
...
Route::get('motorcycleproductdetails', ['middleware'=>'cors',function(){
  date_default_timezone_set("Asia/Singapore");
      ...
      $sql = 'SELECT
              ...
              t_store_products.product_condition,
      ...
      if(!empty($result)){
        ...
        if(!empty($result)){
          ...
          $product_info['brand'] = $result[0]->brand_name;
          ...
          $product_info['condition'] = $result[0]->product_condition;
          $product_info['description'] = $result[0]->description;
...
Route::post('submitapplication',['middleware'=>'cors',function(Request $request){
  ...
  //get all parameters submitted
  //verify all are correct and not empty
  //save the application to table
  $image = request('attachment');
  if(!empty($image)){ //check if has attachment
    if(!empty(request('profile-image'))){
      $profimage = request('profile-image');
      $fileext2 = $profimage->getClientOriginalExtension();
      $proimg = "motors-".str_replace(" ","",request('first-name')).str_replace(" ","",request('last-name')).date("Ymdhis").'.'.$image->getClientOriginalExtension();
      $destinationPath = public_path('../../images/submitted-applications/');
      $profimage->move($destinationPath, $proimg);
    }

    //check extensino file, which should allow image files type only
    $fileext = $image->getClientOriginalExtension();
    $allowedext = array("jpg","png","jpeg","gif","svg"); //jpeg,png,jpg,gif,svg
    if(in_array($fileext,$allowedext)){
      $imgname = "motors-".str_replace(" ","",request('first-name')).str_replace(" ","",request('last-name')).date("Ymdhis").'.'.$image->getClientOriginalExtension();
      $destinationPath = public_path('../../images/submitted-applications/');
      $image->move($destinationPath, $imgname);

      if(!empty(request('first-name')) and !empty(request('last-name')) and !empty(request('personal-mobile')) and !empty(request('user-id')) and !empty(request('product-id'))){
        ...
        $sa->valid_id = $imgname;
        $sa->uploaded_picture = $imgname;

        /*$sa->data_info = request('data-privacy-1');
        $sa->data_privacy = request('data-privacy-2');*/
        $sa->product_id = request('product-id');
        $sa->qty = request('product-qty');
        $sa->terms_id = request('product-terms');
        $sa->user_id = request('user-id');
        $sa->store_id = request('store-id');
        $sa->save();
      }
      else{
        $err_msg = "Please input required fields";
      }
    }
    else{ //error file extension
      $err_msg = "Only images are allowed to be uploaded.";
    }
  }
  else{ //error, should have a file upload
    $err_msg = "Form doesn't have an attachment.";
  }
...
function motordefaultlist(){
  ...
  $sql = 'SELECT
          ...
          INNER JOIN t_store_products ON t_store_products.store_id=t_stores.id
          INNER JOIN t_submitted_products ON t_submitted_products.id=t_store_products.product_id
          WHERE t_store_products.status=1';
